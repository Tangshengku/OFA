...
def main(cfg: DictConfig, **kwargs):
    ...
    for sample in progress:
        ...
        with torch.no_grad():
            result, scores, time, txt_layer, img_layer = eval_step(task, generator, models, sample, **kwargs)
        results += result
        txt_layers += txt_layer
        img_layers += img_layer
        times.append(time) 
    ...
    logger.info("text layers: {}".format(txt_layers/score_cnt))
    logger.info("image layers: {}".format(img_layers/score_cnt))
