...
path=/data2/tsk/checkpoints/stage2_checkpoints/6_task_loss_3/checkpoint.best_cider_1.4300.pt
result_path=../../results/caption
selected_cols=1,4,2
split='test'

CUDA_VISIBLE_DEVICES=3,4,5,6,7 python3 -m torch.distributed.launch --nproc_per_node=5 --master_port=${MASTER_PORT} ../../evaluate.py \
    ${data} \
    --path=${path} \
    --user-dir=${user_dir} \
    --task=caption \
