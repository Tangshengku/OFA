...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TransformerEncoder(args, src_dict, embed_tokens, decomposition=args.decomposition)
...
@register_model("unify_transformer_decomnpose")
class TransformerModelDecompose(FairseqEncoderDecoderModel):
    """
    Transformer model from `"Attention Is All You Need" (Vaswani, et al, 2017)
    <https://arxiv.org/abs/1706.03762>`_.
    ...
    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TransformerEncoder(args, src_dict, embed_tokens, decomposition=True, orthogonal_size=args.orthogonal_size)
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerDecoder(
            ...
            embed_tokens,
            decomposition=True,
            orthogonal_size=args.orthogonal_size,
            no_encoder_attn=getattr(args, "no_cross_attention", False),
        )
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, decomposition=False, orthogonal_size=128):
        self.args = args
    ...
    def build_encoder_layer(self, args, drop_path_rate=0.0):
        layer = TransformerEncoderLayer(args, self.decomposition, self.orthogonal_size, drop_path_rate=drop_path_rate)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False, drop_path_rate=0.0):
        layer = TransformerDecoderLayer(args, no_encoder_attn=no_encoder_attn, decomposition=self.decomposition, orthogonal_size=self.orthogonal_size, drop_path_rate=drop_path_rate)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
...
def base_architecture(args):
    ...
    args.decomposition = getattr(args, "decomposition", False)
    args.orthogonal_size = getattr(args, "orthogonal_size", 128)
